    ...
    public class Game1 : Microsoft.Xna.Framework.Game
    {
        ...
        protected override void Update(GameTime gameTime)
        {
            ...
            for (int i = 0; i < bears.Count; i++)
            {
                for (int j = i + 1; j < bears.Count; j++)
                {
                    CollisionResolutionInfo teddyCollision = CollisionUtils.CheckCollision(gameTime.ElapsedGameTime.Milliseconds,
                        GameConstants.WINDOW_WIDTH, GameConstants.WINDOW_HEIGHT, bears[i].Velocity, bears[i].CollisionRectangle,
            ...
            foreach (TeddyBear bear in bears)
            {
                ...
                foreach (Projectile projectile in projectiles)
                {
                    if (projectile.Active == true && bear.Active == true)
                    {
                        if (projectile.Type == ProjectileType.FrenchFries && bear.CollisionRectangle.Intersects(projectile.CollisionRectangle))
                        {
                            bear.Active = false;
        ...
        private void SpawnBear()
        {
            ...
            float randAngle = RandomNumberGenerator.NextFloat((float)Math.PI * 2);
            Vector2 vspeed = new Vector2(randSpeed * (float)Math.Cos(randAngle), randSpeed * (float)Math.Sin(randAngle));
            // create new bear
            TeddyBear newBear = new TeddyBear(Content, "teddybear", x, y, vspeed, null, null);
